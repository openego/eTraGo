...
if __name__ == "__main__":
    ...
    _ssh_tunnel_path = r"path_to_local_ssh_tunnel_config.json"
    if _ssh_tunnel_path:
    ...
    if not args:
        if (
            _ssh_tunnel
        ):  # only add db_ssh to args if your intention is to connect to a remote database
    ...
    json_path = os.path.join(
        os.path.dirname(os.getcwd()), "etrago", "args.json"
    )
