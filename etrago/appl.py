...
storage_extendable_true = [True]

storage_extendable_false = [False]

storage_extendable_list = len(start)*storage_extendable_false + len(start)*storage_extendable_true
...
while n < 2*len(start):
    ...
    args = {# Setup and Configuration:
            ...
            'results': results_list[n], # state if and where you want to save results as csv: False or /path/tofolder
            'export': False, # state if you want to export the results back to the database
            # Settings:        
            'storage_extendable': storage_extendable_list[n], # state if you want storages to be installed at each node if necessary.
            ...
            'comments':None,
            'switches': False, #state if you use a scenario with switches and want to delete certain switches to avoid subnetworks
            }
    ...
    def etrago(args):
        ...
        p_output = pd.Series(network.generators_t.p.groupby(network.generators.carrier, axis=1).sum().sum(), name='generation')
        price_generators = network.generators.marginal_cost * network.generators_t.p
        p_by_carrier = pd.Series(price_generators.groupby(network.generators.carrier, axis=1).sum().sum(), name='marginal_costs')
        output = pd.concat([p_output, p_by_carrier], axis=1)
        output['costs'] = network.objective
