...
args = {
    ...
    'csv_export': '/home/lukas_wienholt/results/ego-valid',  # save results as csv: False or /path/tofolder
...
#    network.add("Generator", '25617 wind_offshore', bus=25617, carrier='wind_offshore', control='PV', capital_cost='NaN', efficiency = 'NaN', marginal_cost=0, p_nom=900)
    ...
    # reduce biomass capacity to check for flexibility requirements (possibly increase offshore wind accordingly):
    ...
    network.generators.p_nom.loc[(~network.generators.bus.isin(foreign_buses.index)) & (network.generators.carrier == 'biomass')]  = 0.5 * network.generators.p_nom.loc[(~network.generators.bus.isin(foreign_buses.index)) & (network.generators.carrier == 'biomass')]
