...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 16 13:21:52 2025
...
def _calc_storage_expansion(self):
    ...
    return (
        (
            ...
            - self.storage_units.p_nom_min
        )[self.storage_units.p_nom_extendable]
        .groupby(self.storage_units.carrier)
        .sum()
    )
...
def _calc_store_expansion(self):
    ...
    """
    network = self
    return (network.stores.e_nom_opt - network.stores.e_nom_min)[
        network.stores.e_nom_extendable
    ]
...
def _calc_sectorcoupling_link_expansion(self):
    ...
    """
    network = self
    ext_links = self.links[self.links.p_nom_extendable]
...
def system_costs_germany(self):
    ...
    network_de = german_network(self.network)
    network = self
