...
class Disaggregation:
    def __init__(self, original_network, clustered_network, clustering, skip=()):
        """
    ...
    def reindex_with_prefix(self, dataframe, prefix=None):
        if prefix is None:
            ...
            dataframe.set_index(
                dataframe.index.map(lambda x: self.idx_prefix + x), inplace=True
            )
    ...
    def construct_partial_network(self, cluster, scenario):
        ...
        for line_type in line_types:
            ...
            if not left_external_connectors.empty:
                ...
                left_external_connectors.loc[:, "bus0"] = left_external_connectors.loc[
                    :, "bus0"
                ].apply(f)
        ...
        externals_to_insert = self.clustered_network.buses[
            self.clustered_network.buses.index.isin(
                map(lambda x: x[0][len(self.idx_prefix) :], external_buses.values)
            )
        ]
        ...
        for bustype in bus_types:
            ...
            setattr(
                partial_network,
                bustype,
                filter_buses(getattr(self.original_network, bustype), buses_in_lines),
            )
    ...
    def solve(self, scenario, solver):
        ...
        self.stats = {
            "clusters": pd.DataFrame(
                index=sorted(clusters), columns=["decompose", "spread", "transfer"]
            )
        }
        ...
        for i, cluster in enumerate(sorted(clusters)):
            ...
            print("Decomposed in ", self.stats["clusters"].loc[cluster, "decompose"])
            t = time.time()
            profile.enable()
            ...
            print(
                "Result distributed in ", self.stats["clusters"].loc[cluster, "spread"]
            )
            profile.enable()
            t = time.time()
    ...
    def transfer_results(
        ...
        partial_network,
        externals,
        bustypes=["loads", "generators", "stores", "storage_units", "shunt_impedances"],
        series=None,
    ):
        for bustype in bustypes:
            ...
            for key in (
                orig_buses.keys()
                if series is None
                else (k for k in orig_buses.keys() if k in series.get(bustype, {}))
            ):
    ...
    def _validate_disaggregation_generators(self, cluster, f):
        def extra_functionality(network, snapshots):
            ...
            def construct_constraint(model, snapshot, carrier):
                ...
                generator_p = [
                    ...
                    for x in generators.loc[
                        (generators.bus == cluster) & (generators.carrier == carrier)
                    ].index
                ]
                if not generator_p:
            ...
            network.model.validate_generators = Constraint(
                list(snapshots), set(generators.carrier), rule=construct_constraint
            )
            ...
            for bustype, bustype_pypsa, suffixes in [
                ...
                for suffix in suffixes:
                    ...
                    def construct_constraint(model, snapshot):
                        # TODO: Optimize
                        buses_p = [
                            getattr(model, bustype + "_p" + suffix)[(x, snapshot)]
                            for x in generators.loc[(generators.bus == cluster)].index
                        ]
                        if not buses_p:
                            return Constraint.Feasible
                        ...
                        cluster_buses = getattr(self.clustered_network, bustype_pypsa)[
                            (
                                getattr(self.clustered_network, bustype_pypsa).bus
                                == cluster
                            )
                        ]
                        sum_clustered_p = sum(
                            getattr(self.clustered_network, bustype_pypsa + "_t")[
                                "p"
                            ].loc[snapshot, c]
...
class UniformDisaggregation(Disaggregation):
    def solve_partial_network(self, cluster, partial_network, scenario, solver=None):
        ...
        weights = {
            ...
            "q": (
                ("p_nom_opt",)
                if (getattr(self.clustered_network, "allocation", None) == "p_nom")
        ...
        for bustype in bustypes:
            ...
            for group in groups:
                ...
                if clb.iloc[0].at["p_nom_extendable"]:
                    ...
                    pnb.loc[:, "p_nom_opt"] = (
                        clb.iloc[0].at["p_nom_opt"] * pnb_p_nom_max / p_nom_max_global
                    )
                ...
                for s in bustypes[bustype]["series"]:
                    ...
                    loc = (slice(None),) if any(timed(w) for w in weights[s]) else ()
        ...
        if disagg:
            if disagg == "mini":
                disaggregation = MiniSolverDisaggregation(
                    self.disaggregated_network, self.network, self.clustering, skip=skip
                )
            elif disagg == "uniform":
