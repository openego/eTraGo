...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul  7 13:26:37 2023
...
args = {
    ...
    "solver_options":{
        ...
        "crossover":0,
        "logFile": "eGon2035_solver_etrago.log",
        "threads": 4,
        "method":2, 
        "BarHomogeneous":1
        },
    ...
    # Export options:
    "lpfile": '/home/egon/Till/eTraGo/lpfile.lp',  # save pyomo's lp file: False or /path/to/lpfile.lp
    ...
    "extendable": {
        ...
        "upper_bounds_grid": {  # Set upper bounds for grid expansion
            ...
            "grid_max_abs_D": {  # absolute capacity per voltage level
                "380": {"i": 1020, "wires": 10, "circuits": 4},
                "220": {"i": 1020, "wires": 10, "circuits": 4},
                "110": {"i": 1020, "wires": 10, "circuits": 2},
                "dc": 0,
            },
    ...
    "network_clustering": {
        ...
        "cluster_foreign_gas": True,  # take foreign CH4 buses into account, True or False
...
def run_etrago(args, json_path):
    ...
    #from etrago import Etrago
    etrago = Etrago(csv_folder_name='/home/student/Documents/Masterthesis/eTraGo_szenarien/erstesclusterbeide')
    etrago.skip_snapshots()
    market = Etrago(csv_folder_name='/home/student/Documents/Masterthesis/eTraGo_szenarien/marketfinal_ols')
   
    
    #Fehlerhafte Generatoren droppen
    etrago.network.generators=etrago.network.generators.drop(['7 wind_offshore', '38 wind_offshore' ])
    etrago.network.generators_t.p_max_pu= etrago.network.generators_t.p_max_pu.drop(['7 wind_offshore', '38 wind_offshore'], axis=1)
    ...
    etrago.network.links.p_nom.loc[etrago.network.links.carrier == 'central_gas_boiler'] *= 1000
    ...
    market_gen_carrier_list = market_gen_carrier_list.drop( market_gen_carrier_list.loc[(market_gen_carrier_list =="CH4") | (market_gen_carrier_list =="CH4_NG") | (market_gen_carrier_list =="CH4_biogas") | (market_gen_carrier_list =="geo_thermal") | (market_gen_carrier_list =="solar_thermal_collector") | (market_gen_carrier_list =="load shedding")].index).unique().tolist()
    
    #market_gen_carrier_list =="load shedding")
    ...
    a=a.T
    etrago.network.generators.loc[tst2.index]=etrago.network.generators.reindex(tst2.index)
    ...
    stores_e_nom_max =  pd.DataFrame({'max_value': etrago.network.stores_t.p.max(axis=0).combine(etrago.network.stores_t.p.max(axis=0), max)}, index=etrago.network.stores_t.p.columns)
    stores_test = etrago.network.stores.e_nom.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')].to_frame()
    stores_e_nom_index = stores_e_nom_max.loc[stores_e_nom_max.index[stores_e_nom_max.index.isin(stores_test.index)]]
    stores_test.loc[stores_e_nom_index.index] = stores_e_nom_index
    etrago.network.stores.loc[stores_test.index, stores_test.columns] = stores_test
    
    #Versuch das store Problem noch zu lösen, leider nicht mehr geschafft
    
    #etrago.network.stores_t.e_max_pu = etrago.network.stores_t.e_max_pu.join((etrago.network.stores_t.p.T.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')]).T / etrago.network.stores.e_nom.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')])
    #etrago.network.stores_t.e_min_pu = etrago.network.stores_t.e_min_pu.join((etrago.network.stores_t.p.T.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')]).T / etrago.network.stores.e_nom.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')])
    #etrago.network.stores.e_nom_extendable.loc[(etrago.network.stores.carrier == 'rural_heat_store') | (etrago.network.stores.carrier == 'central_heat_store') | (etrago.network.stores.carrier == 'H2_overground')] = False
    ...
    ch4_h2_link_load = etrago.network.loads_t.p_set.T.loc[etrago.network.loads.bus.isin(ch4_h2_link.bus0)].groupby(etrago.network.loads.bus).sum()
    ...
    rus = geopandas.read_file('/home/student/Documents/Masterthesis/EU Grenzen/Nut/Russland/Russland.shp')
    rus = rus[['SOV_A3','geometry']]
    rus = rus.rename(columns={'SOV_A3': 'FID'})
    rus['NUTS_NAME'] = 'Russland'
    europe = europe.to_crs(4326)
    europe = europe[['NUTS_NAME','FID','geometry']]
    europe = europe.append(rus)
    ...
    #Redispatchkomponenten vorbereiten / Generators
    ...
    #festsetzen der Zeitreihen für normale generators
    g_p = (etrago.network.generators_t.p.T.loc[ ~((etrago.network.generators.carrier == 'CH4') | (etrago.network.generators.carrier == 'geo_thermal') | (etrago.network.generators.carrier == 'solar_thermal_collector') | (etrago.network.generators.carrier == 'central_biomass_CHP_heat') | (etrago.network.generators.carrier == 'CH4_NG') | (etrago.network.generators.carrier == 'CH4_biogas'))].T/ etrago.network.generators.p_nom.loc[~((etrago.network.generators.carrier == 'CH4') | (etrago.network.generators.carrier == 'geo_thermal') | (etrago.network.generators.carrier == 'solar_thermal_collector') | (etrago.network.generators.carrier == 'central_biomass_CHP_heat') | (etrago.network.generators.carrier == 'CH4_NG') | (etrago.network.generators.carrier == 'CH4_biogas') | (etrago.network.generators.carrier == 'load shedding'))])
    ...
    #Redispatchkomponenten vorbereiten / links und links die in gen gewandelt werden 
    l_up = etrago.network.links.loc[etrago.network.links.carrier =='central_heat_pump'].copy()
    l_up_gen = etrago.network.links.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].copy()
   
    l_down = l_up.copy()
    l_down_gen = l_up_gen.copy()
    #l_down = l_down.loc[((l_down['FID0'] =='DE') & (l_down['FID1'] =='DE')).T]
    #l_up = l_up.loc[((l_up['FID0'] =='DE') & (l_up['FID1'] =='DE')).T]
    #l_down_gen = l_down_gen.loc[((l_down_gen['FID0'] =='DE') & (l_down_gen['FID1'] =='DE')).T]
    #l_up_gen = l_up_gen.loc[((l_up_gen['FID0'] =='DE') & (l_up_gen['FID1'] =='DE')).T]
    l_up.index = l_up.index.map(lambda x: x + " ramp up")
    l_down.index = l_down.index.map(lambda x: x + " ramp down")
    l_up_gen.index = l_up_gen.index.map(lambda x: x + " ramp up")
    l_down_gen.index = l_down_gen.index.map(lambda x: x + " ramp down")
   
    
    up_l = (
            etrago.network.links.loc[(etrago.network.links.carrier =='central_heat_pump')].p_nom - etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier =='central_heat_pump')].T).clip(0) / etrago.network.links.loc[(etrago.network.links.carrier =='central_heat_pump')].p_nom
    up_l_gen = (
            etrago.network.links.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].p_nom - etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].T).clip(0) / etrago.network.links.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].p_nom
    
    
    up_l=up_l.dropna(axis='columns')
    up_l.index=up_l.index.astype('datetime64[ns]')
    #up_l= up_l.T.loc[((etrago.network.links.FID0 == 'DE') & (etrago.network.links.FID1 == 'DE'))].T
     
    up_l_gen=up_l_gen.dropna(axis='columns')
    up_l_gen.index=up_l_gen.index.astype('datetime64[ns]')
    #up_l_gen= up_l_gen.T.loc[((etrago.network.links.FID0 == 'DE') & (etrago.network.links.FID1 == 'DE'))].T
   
    
    down_l =  -etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier =='central_heat_pump')].T / etrago.network.links.loc[(etrago.network.links.carrier =='central_heat_pump')].p_nom
    down_l= down_l.dropna(axis='columns')
    down_l.index=down_l.index.astype('datetime64[ns]')
    #down_l= down_l.T.loc[((etrago.network.links.FID0 == 'DE') & (etrago.network.links.FID1 == 'DE'))].T
    
    down_l_gen =  -etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].T / etrago.network.links.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].p_nom
    down_l_gen= down_l_gen.dropna(axis='columns')
    down_l_gen.index=down_l_gen.index.astype('datetime64[ns]')
    #down_l_gen = down_l_gen.T.loc[(etrago.network.links.FID0 == 'DE') & (etrago.network.links.FID1 == 'DE')].T
    
    
    up_l.columns = up_l.columns.map(lambda x: x + " ramp up")
    down_l.columns = down_l.columns.map(lambda x: x + " ramp down")
    
    up_l_gen.columns = up_l_gen.columns.map(lambda x: x + " ramp up")
    down_l_gen.columns = down_l_gen.columns.map(lambda x: x + " ramp down")
    
   
    
    #festsetzen der Zeitreihen für normale links
    l_p = (etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier =='central_heat_pump')].T/etrago.network.links.loc[(etrago.network.links.carrier =='central_heat_pump')].p_nom).T
    l_p = l_p.dropna(axis='rows')
    
    l_p_gen = (etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].T/etrago.network.links.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP')].p_nom).T
    l_p_gen = l_p_gen.dropna(axis='rows')
   
    etrago.network.links_t.p_min_pu = etrago.network.links_t.p_min_pu.join(l_p.T)
    etrago.network.links_t.p_min_pu = etrago.network.links_t.p_min_pu.join(l_p_gen.T)
      
    etrago.network.links_t.p_max_pu= (etrago.network.links_t.p_max_pu.T.loc[~(etrago.network.links.carrier =='central_heat_pump')]).T
    etrago.network.links_t.p_max_pu = (etrago.network.links_t.p_max_pu.T.loc[~((etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP'))]).T
    etrago.network.links_t.p_max_pu = etrago.network.links_t.p_max_pu.join(l_p.T)
    etrago.network.links_t.p_max_pu = etrago.network.links_t.p_max_pu.join(l_p_gen.T)
    
    #p_nom=0 maximal Werte aus Zeitreihen zuweisen
    ...
    #links zur Lösbarkeit in Gassektor Redispatchkompo erstellen
    l_up_diss = etrago.network.links.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power') ].copy()
    l_down_diss = l_up_diss.copy()
    l_up_diss.index = l_up_diss.index.map(lambda x: x + " ramp up")
    l_down_diss.index = l_down_diss.index.map(lambda x: x + " ramp down")
    
    up_l_diss = (
            etrago.network.links.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].p_nom - etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].T).clip(0) / etrago.network.links.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].p_nom        
    up_l_diss=up_l_diss.dropna(axis='columns')
    up_l_diss.index=up_l_diss.index.astype('datetime64[ns]')        
            
    down_l_diss =  -etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].T / etrago.network.links.loc[(etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].p_nom
    down_l_diss= down_l_diss.dropna(axis='columns')
    down_l_diss.index=down_l_diss.index.astype('datetime64[ns]')        
            
    up_l_diss.columns = up_l_diss.columns.map(lambda x: x + " ramp up")
    down_l_diss.columns = down_l_diss.columns.map(lambda x: x + " ramp down")       
                
    l_p_diss = (etrago.network.links_t.p0.T.loc[(etrago.network.links.carrier == 'central_resistive_heater') | (etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')].T/ etrago.network.links.p_nom.loc[(etrago.network.links.carrier == 'central_resistive_heater') | (etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'H2_to_power')])
    etrago.network.links_t.p_min_pu = etrago.network.links_t.p_min_pu.join(l_p_diss) 
    etrago.network.links_t.p_max_pu = etrago.network.links_t.p_max_pu.join(l_p_diss)
    etrago.network.links_t.p_max_pu.loc[:, etrago.network.links_t.p_max_pu.isna().any()]=0
    etrago.network.links_t.p_min_pu.loc[:, etrago.network.links_t.p_min_pu.isna().any()]=0
        
    l_up_gen = l_up_gen.rename(columns={'bus1': 'bus', 'FID1': 'FID', 'zone1': 'zone'})
    columns_to_drop = ['scn_name','bus0','length', 'geom','topo', 'type', 'terrain_factor','FID0', 'zone0' ]
    l_up_gen = l_up_gen.drop(columns_to_drop, axis=1)
    #Kosten der naturalgas gens addieren
    l_up_gen.marginal_cost += 27.56
    l_up_gen.p_nom = l_up_gen.p_nom*l_up_gen.efficiency
    l_up_gen.efficiency = 1
    
    # Struktur an generators anpassen
    l_down_gen = l_down_gen.rename(columns={'bus1': 'bus', 'FID1': 'FID', 'zone1': 'zone'})
    l_down_gen = l_down_gen.drop(columns_to_drop, axis=1)
    l_down_gen.marginal_cost += 27.56 
    l_down_gen.p_nom = l_down_gen.p_nom*l_down_gen.efficiency
    l_down_gen.efficiency = 1
    
    # Redispatch Komponenten hinzufügen
    ...
    etrago.network.madd("Link", l_up_diss.index, p_max_pu=up_l_diss, **l_up_diss.drop(["p_max_pu"], axis=1))
    etrago.network.madd("Link", l_down_diss.index, p_min_pu=down_l_diss, p_max_pu=0, **l_down_diss.drop(["p_max_pu", "p_min_pu"], axis=1))
    ...
    etrago.network.madd("Generator", l_up_gen.index, p_max_pu=up_l_gen, **l_up_gen.drop(["p_max_pu"], axis=1))
    etrago.network.madd("Generator", l_down_gen.index, p_min_pu=down_l_gen, p_max_pu=0, **l_down_gen.drop(["p_max_pu", "p_min_pu"], axis=1))
    
    etrago.network.links.p_nom_extendable.loc[(etrago.network.links.carrier == 'CH4_to_H2') | (etrago.network.links.carrier == 'power_to_H2') | (etrago.network.links.carrier == 'H2_to_power') | (etrago.network.links.carrier == 'H2_to_CH4')] = False
    ...
    # Länderkosten media für Knoten ermitteln
    tst = pd.DataFrame()
    ...
    etrago.network.links_t.marginal_cost.loc[(etrago.network.links.carrier == 'central_heat_pump') | (etrago.network.links.carrier == 'central_gas_CHP_heat')| (etrago.network.links.carrier == 'H2_feedin') | (etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'power_to_H2') | (etrago.network.links.carrier == 'CH4_to_H2')] = tst.loc[laender_link.loc[(etrago.network.links.carrier == 'central_heat_pump') | (etrago.network.links.carrier == 'central_gas_CHP_heat') | (etrago.network.links.carrier == 'H2_feedin') | (etrago.network.links.carrier == 'H2_to_CH4') | (etrago.network.links.carrier == 'power_to_H2') | (etrago.network.links.carrier == 'CH4_to_H2') ].FID0].values
    #breakpoint()
    #laender_link= laender_link.loc[~((laender_link.FID0 == 'UK') | (laender_link.FID1 == 'UK'))]
    etrago.network.links_t.marginal_cost.loc[laender_link.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP') | (etrago.network.links.carrier == 'H2_to_power')].index] = tst.loc[laender_link.loc[(etrago.network.links.carrier == 'OCGT') | (etrago.network.links.carrier == 'central_gas_CHP') | (etrago.network.links.carrier == 'industrial_gas_CHP') | (etrago.network.links.carrier == 'H2_to_power') ].FID1].values
    ...
    #breakpoint()
    #laender_gen= laender_gen.loc[~(laender_gen.FID == 'RU')]
    ...
    #kleine Werte zu 0 setzen
    etrago.network.generators_t.p = etrago.network.generators_t.p.where( etrago.network.generators_t.p >= 1e-5,0)
    etrago.network.stores_t.p = etrago.network.stores_t.p.where( etrago.network.stores_t.p >= 1e-5,0)
    etrago.network.storage_units_t.p = etrago.network.storage_units_t.p.where( etrago.network.storage_units_t.p >= 1e-5,0)
    etrago.network.links_t.p0 = etrago.network.links_t.p0.where( etrago.network.links_t.p0 >= 1e-5,0)
    etrago.network.links_t.p1 = etrago.network.links_t.p1.where( etrago.network.links_t.p1 >= 1e-5,0)
    
    etrago.network.generators_t.marginal_cost = etrago.network.generators_t.marginal_cost.where((etrago.network.generators_t.marginal_cost < -1e-5) | (etrago.network.generators_t.marginal_cost > 1e-5) ,0) 
    etrago.network.stores_t.marginal_cost = etrago.network.stores_t.marginal_cost.where((etrago.network.stores_t.marginal_cost < -1e-5) | (etrago.network.stores_t.marginal_cost > 1e-5),0)
    etrago.network.storage_units_t.marginal_cost = etrago.network.storage_units_t.marginal_cost.where((etrago.network.storage_units_t.marginal_cost < -1e-5) | (etrago.network.storage_units_t.marginal_cost > 1e-5) ,0)
    etrago.network.links_t.marginal_cost = etrago.network.links_t.marginal_cost.where((etrago.network.links_t.marginal_cost < -1e-5) | (etrago.network.links_t.marginal_cost > 1e-5) ,0)
    ...
    #etrago.export_to_csv('/home/egon/Till/eTraGo/netzfinal')
    etrago.export_to_csv('/home/egon/Till/eTraGo/netzfinal100_deu')
    #from etrago import Etrago
    #etrago = Etrago(csv_folder_name='/home/egon/Till/eTraGo/netzvorshedding')
