...
def add_missing_components(network):
    ...
    bus0m = to_shape(network.buses.geom['23648'])
    bus1m = to_shape(network.buses.geom['16573'])

    # trafo geom/topo
    network.transformers.set_value(new_trafo, 'geom', from_shape(MultiLineString([LineString([bus0m,bus1m])]),4326))
    network.transformers.set_value(new_trafo, 'topo', from_shape(LineString([bus0m,bus1m]),4326))
    ...
    def add_110kv_line(bus0, bus1, overhead=False):
        ...
        bus0 = to_shape(network.buses.geom[bus0])
        bus1 = to_shape(network.buses.geom[bus1])
        ...
        # line geom/topo
        network.lines.set_value(new_line, 'geom', from_shape(MultiLineString([LineString([bus0,bus1])]),4326))
        network.lines.set_value(new_line, 'topo', from_shape(LineString([bus0,bus1]),4326))
    ...
    bus0s = to_shape(network.buses.geom['25766'])
    bus1s = to_shape(network.buses.geom['18967'])
    
    # trafo geom/topo
    network.transformers.set_value(new_trafo, 'geom', from_shape(MultiLineString([LineString([bus0s,bus1s])]),4326))
    network.transformers.set_value(new_trafo, 'topo', from_shape(LineString([bus0s,bus1s]),4326))
