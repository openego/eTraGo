...
def german_network(self):
    ...
    """
    keep_cntr = ["DE"]
    new_idx = self.network.buses[self.network.buses.country.isin(keep_cntr)].index
    ...
    new_network.mremove(
        "Bus",
        new_network.buses[~new_network.buses.index.isin(new_idx)].index)
    
    new_network.mremove(
        "Line",
        new_network.lines[~new_network.lines.index.isin(            
            new_network.lines[
                (
    ...
    new_network.mremove(
        "Link",
        new_network.links[~new_network.links.index.isin(            
            new_network.links[
                (
    ...
    new_network.mremove(
        "Transformer",
        new_network.transformers[~new_network.transformers.index.isin(            
    ...
    new_network.mremove(
        "Generator",
        new_network.generators[~new_network.generators.index.isin(            
    ...
    new_network.mremove(
        "Load",
        new_network.loads[~new_network.loads.index.isin(            
            new_network.loads[
                new_network.loads.bus.isin(new_idx)                    
            ].index)].index)
    
    new_network.mremove(
        "Store",
        new_network.stores[~new_network.stores.index.isin(            
            new_network.stores[
                new_network.stores.bus.isin(new_idx)                    
            ].index)].index)
    
    new_network.mremove(
        "StorageUnit",
        new_network.storage_units[~new_network.storage_units.index.isin(            
...
def system_costs_germany(self):
    ...
    for c in network_de.iterate_components():
        ...
        if c.name in network_de.one_port_components:
            if "marginal_cost" in c.df.columns:
                marginal_cost += c.pnl.p.mul(c.df.marginal_cost).mul(
                    network_de.snapshot_weightings.generators, axis=0).sum().sum()

        else:
            if "marginal_cost" in c.df.columns:
                marginal_cost += c.pnl.p0.mul(c.df.marginal_cost).mul(
                    network_de.snapshot_weightings.generators, axis=0).sum().sum()
        if not c.name in ["Bus", "Load", "LineType", "TransformerType", "Carrier"]:
            invest_cost += ((
                c.df[c.df[f"{value}_nom_extendable"]][f"{value}_nom_opt"] - 
                c.df[c.df[f"{value}_nom_extendable"]][f"{value}_nom_min"]
                )*c.df[c.df[f"{value}_nom_extendable"]]["capital_cost"]).sum()
    ...
    export_positive = self.network.links_t.p0[links_export.index].clip(lower=0).mul(
        self.network.snapshot_weightings.generators, axis=0).mul(
        self.network.buses_t.marginal_price[links_export.bus1].values,
        ).sum().sum()
    
    export_negative = self.network.links_t.p0[links_export.index].clip(upper=0).mul(
        self.network.snapshot_weightings.generators, axis=0).mul(
        self.network.buses_t.marginal_price[links_export.bus1].values,
        ).mul(-1).sum().sum()
    ...
    import_positive = self.network.links_t.p0[links_import.index].clip(lower=0).mul(
        self.network.snapshot_weightings.generators, axis=0).mul(
        self.network.buses_t.marginal_price[links_import.bus1].values,
        ).sum().sum()
    
    import_negative = self.network.links_t.p0[links_import.index].clip(upper=0).mul(
        self.network.snapshot_weightings.generators, axis=0).mul(
        self.network.buses_t.marginal_price[links_import.bus1].values,
        ).mul(-1).sum().sum() 
