...
def fixed_waste_chp_ratio_linopy(network, snapshots):
    ...
    if (network.links.carrier == "central_waste_CHP_heat").any():
        electric = network.links.index[electric_bool]
        heat = network.links.index[heat_bool]

        ch4_nodes_with_waste_chp = network.buses.loc[
            network.links.loc[electric, "bus0"].values
        ].index.unique()

        for i in ch4_nodes_with_waste_chp:
            elec_chp = network.links[
                (network.links.carrier == "central_waste_CHP")
                & (network.links.bus0 == i)
            ].index

            heat_chp = network.links[
                (network.links.carrier == "central_waste_CHP_heat")
                & (network.links.bus0 == i)
            ].index

            for snapshot in snapshots:
                dispatch_heat = get_var(network, "Link", "p").loc[snapshot, heat_chp].sum()
                dispatch_elec = get_var(network, "Link", "p").loc[snapshot, elec_chp].sum()

                define_constraints(
                    network,
                    (dispatch_heat - fixed_ratio * dispatch_elec),
                    "=",
                    0,
                    "Link",
