...
def _max_curtailment(self, network, snapshots):
    ...
    res = list(network.generators.index[(network.generators.carrier.isin(
                renewables))& (network.generators.bus.astype(str).isin(
    ...
    def _rule(m):
        ...
        return (re_n >= (1-self.args['extra_functionality']['max_curtailment'])
                                    * res_potential)

    setattr(network.model, "max_curtailment", Constraint(res, rule=_rule))

def _max_curtailment_per_gen(self, network, snapshots):
    renewables = ['wind_onshore', 'wind_offshore', 'solar']
